...
class NORGate(Scene):
    def construct(self):
        ...
        arc0, arc1, arc2 = ArcBetweenPoints(a, b, radius=-2, **arc_conf), ArcBetweenPoints(b, c, radius=2, **arc_conf), ArcBetweenPoints(c, a, radius=2, **arc_conf)
        dots = Group(*dots)
        circle = Circle(1).set_color(WHITE).scale(0.15).set_stroke(width=5) # type: ignore
        gate = Group(arc0, arc1, arc2, dots).center()
        nand_gate = Group(gate, circle).arrange_in_grid(2,1, row_heights=[-2.55,0])

        dot0, dot1, dot2, dot3, dot4, dot5 = Dot([-0.5, -0.8, 0]), Dot(bits[0]), Dot([0.5, -0.8, 0]), Dot(bits[1]), Dot([0, 1.06, 0]), Dot(bits[2])
